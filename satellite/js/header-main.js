...
(function () {
  ...
  function applyState() {
    ...
    // 80px 로고 축소: 검색 열림 시에도 축소 유지
    document.body.classList.toggle('scrolled', searchOpen || y >= 80);

    // 헤더/링크/로고 색상 반전: 검색 열림 시에도 반전 유지
    header.classList.toggle('scrolled', searchOpen || y > 1080);
